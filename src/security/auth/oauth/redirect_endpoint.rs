...
use crate::security::auth::oauth::{authorization_check, OAUTH_AUTHORIZATION_REQUEST_STATE_SESSION_KEY, OAUTH_SESSION_KEY, OAuthAuthorizationRequestState, OAuthSession, OAuthSessionTokens};
    ...
    if let Ok(authorization_response) = access_token_request.send().await {
        ...
        match authorization_check(&session_tokens).await {
            ...
            Some(validated_tokens) => {
                ...
                if let Err(e) = session.insert(USER_SESSION_KEY, User::from(validated_tokens.id_token_claims())) {
...
fn restore_authorization_request_state(session: &Session, state: &str) -> Option<(String, String)> {
    match session.get::<OAuthAuthorizationRequestState>(OAUTH_AUTHORIZATION_REQUEST_STATE_SESSION_KEY).ok() {
        Some(Some(authorization_request_state))
            if authorization_request_state.state() == state => {
                Some((
