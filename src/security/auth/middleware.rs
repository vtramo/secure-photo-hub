...
pub async fn authentication_middleware_oauth2_cookie(
    ...
    match session.get::<OAuthSession>(OAUTH_SESSION_KEY).unwrap_or(None) {
        None if req.path() != oidc_redirect_uri_path => {
            ...
            Ok(req.into_response(redirect_to_auth_server).map_into_left_body())
        },
        ...
        Some(oauth_session) => {
            ...
            if oauth_session.is_access_token_expired() {
                ...
                if let Some(validated_tokens) = authorization_check(&session_tokens, oidc_config.jwks(), oidc_config.client_id()).await {
